...
pub const REWA_TO_WHITELIST: u64 = 1;
    ...
    LiquidStakingContractObjBuilder: 'static + Copy + Fn() -> liquid_staking::ContractObj<DebugApi>,
{
    pub fn deploy_staking_contract(
        ...
        let rewa_balance_biguint = &Self::exp18(rewa_balance);
        let deposit_amount =
            &Self::exp18(rewa_balance - REWA_TO_WHITELIST - FIRST_ADD_LIQUIDITY_AMOUNT);
        let total_staked_biguint = Self::exp18(total_staked);
        let delegation_contract_cap_biguint = Self::exp18(delegation_contract_cap);

        self.b_mock
            .set_rewa_balance(owner_address, rewa_balance_biguint);
    ...
    pub fn delegate_rewards_check_insufficient(&mut self, caller: &Address) {
        ...
        self.b_mock
            ...
            .assert_error(4, "Old claimed rewards must be greater than 1 REWA");
    }
    ...
    pub fn setup_new_user(&mut self, rewa_token_amount: u64) -> Address {
        let rust_zero = rust_biguint!(0);

        let new_user = self.b_mock.create_user_account(&rust_zero);
        self.b_mock
            .set_rewa_balance(&new_user, &Self::exp18(rewa_token_amount));
        new_user
    }

    pub fn check_user_balance(&self, address: &Address, token_id: &[u8], token_balance: u64) {
        self.b_mock
            .check_dcdt_balance(address, token_id, &Self::exp18(token_balance));
    }
    ...
    pub fn check_user_rewa_balance(&self, address: &Address, token_balance: u64) {
        self.b_mock
            .check_rewa_balance(address, &Self::exp18(token_balance));
    }

    pub fn check_user_rewa_balance_denominated(&self, address: &Address, token_balance: u128) {
        self.b_mock
            .check_rewa_balance(address, &num_bigint::BigUint::from(token_balance));
    }

    pub fn check_contract_storage(
        &mut self,
        ls_token_supply: u64,
        virtual_rewa_reserve: u64,
        rewards_reserve: u64,
    ) {
        self.b_mock
            .execute_query(&self.sc_wrapper, |sc| {
                ...
                assert_eq!(
                    sc.virtual_rewa_reserve().get(),
                    Self::to_managed_biguint(Self::exp18(virtual_rewa_reserve))
                );
                assert_eq!(
