...
const DELEGATION_MOCK_CONTRACT_CODE: &str = "../delegation-mock/output/delegation-mock.drtsc.json";
...
pub async fn liquid_staking_cli() {
    ...
    match cmd.as_str() {
        ...
        "getVirtualRewaReserve" => interact.virtual_rewa_reserve().await,
...
impl ContractInteract {
    pub async fn new(config: Config) -> Self {
        ...
        let contract_code = BytesValue::interpret_from(
            "drtsc:../output/liquid-staking.drtsc.json",
    ...
    pub async fn deploy_delegation_contract(&mut self) {
        let contract_code = DrtscPath::new(&self.delegation_mock_contract_code);
    ...
    pub async fn add_liquidity(&mut self) {
        let rewa_amount = BigUint::<StaticApi>::from(1_000_000_000_000_000_001u64);
    ...
    pub async fn virtual_rewa_reserve(&mut self) {
        let result_value = self
            .interactor
            .query()
            .to(self.state.current_address())
            .typed(proxy::LiquidStakingProxy)
            .virtual_rewa_reserve()
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn whitelist_delegation_contract(&mut self) {
        let rewa_amount = BigUint::<StaticApi>::from(1_000_000_000_000_000_000u64);
